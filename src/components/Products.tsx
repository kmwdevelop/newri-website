      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {PRODUCTS.map((p, i) => (
            ...
            >
              <div
                className={`group relative flex h-full flex-col justify-between overflow-hidden rounded-3xl ${p.bg} p-7 shadow-sm transition-all hover:-translate-y-1 hover:shadow-lg`}
              >
                <div className="flex items-start justify-between">
                  <span className="rounded-full bg-white/70 px-3 py-1 text-[11px] font-bold tracking-wide text-brown-soft">
                    {p.tag}
                  </span>
                ...
                <div className="mt-8">
                  <h3 className="text-xl font-bold text-brown">{p.name}</h3>
                  <p className="mt-2 max-w-xs text-sm leading-relaxed text-brown-soft">
                    {p.desc}
                  </p>
                  <p className="mt-4 text-sm font-bold text-brown">
                    {p.price ?? "가격 문의"}
                  </p>
                </div>
              </div>
        ...
        <p className="mt-8 text-center text-xs text-brown-muted">
